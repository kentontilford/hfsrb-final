const S = { data: null, charts: {}, sort: { county: {key:'count', dir:'desc'}, region: {key:'count', dir:'desc'} } };
...
function update(){
  const y = el('#year').value; const t = el('#type').value;
...
function destroy(){ for (const k in S.charts){ try{ S.charts[k].destroy(); }catch{} } S.charts={} }
function draw(sel, title, obj){
  const labels = Object.keys(obj||{}); const data = labels.map(k=> obj[k]);
  destroyChart(sel);
  const ctx = el(sel).getContext('2d');
  S.charts[sel] = new Chart(ctx, { type:'bar', data:{ labels, datasets:[{ data }] }, options:{ plugins:{ title:{ display:true, text:title }, legend:{ display:false } }, scales:{ y:{ beginAtZero:true } } } });
}
function destroyChart(sel){ try{ S.charts[sel]?.destroy(); }catch{} }
